...
from besser.BUML.metamodel.structural import (
    ...
    UNLIMITED_MAX_MULTIPLICITY, Constraint, AssociationClass
)
...
from besser.utilities.web_modeling_editor.backend.services.sugiyama_layout import SugiyamaLayout, detect_collisions, optimize_path_routing
import inspect
...
def domain_model_to_json(domain_model):
    ...
    grid_size = {
        "x_spacing": 350,  # Space between elements horizontally (increased from 300)
        "y_spacing": 250,  # Space between elements vertically (increased from 200)
        ...
        if generalization.general in class_id_map and generalization.specific in class_id_map:
            relationships[rel_id] = {
                ...
                "source": {
                    ...
                    "direction": "Up",  # Placeholder, will be recalculated after layout
                    "bounds": {
                        "x": 0,
                        "y": 0,
                        "width": 0,
                ...
                "target": {
                    ...
                    "direction": "Down",  # Placeholder, will be recalculated after layout
                    "bounds": {
                        "x": 0,
                        "y": 0,
                        "width": 0,
    ...
    # Apply Sugiyama-style hierarchical layout algorithm (like PlantUML)
    layout_engine = SugiyamaLayout(
        layer_spacing=120.0,     # Vertical spacing between layers (reduced further for compactness)
        node_spacing=200.0,      # Horizontal spacing between nodes (reduced further for compactness)
        min_padding=30.0         # Minimum padding around nodes
    )

    # Optimize element positions using hierarchical layering
    # This also calculates relationship directions with inheritance children awareness
    elements, relationships = layout_engine.calculate_layout(elements, relationships)

    # Update relationship connection points and paths based on the directions calculated by Sugiyama
    for rel_id, rel in relationships.items():
        if rel.get('type') in ['ClassBidirectional', 'ClassUnidirectional', 'ClassComposition', 'ClassAggregation', 'ClassInheritance']:
            source_elem_id = rel.get('source', {}).get('element')
            target_elem_id = rel.get('target', {}).get('element')

            if source_elem_id in elements and target_elem_id in elements:
                source_element = elements[source_elem_id]
                target_element = elements[target_elem_id]

                # Get the directions calculated by Sugiyama (already considers inheritance children)
                source_dir = rel['source']['direction']
                target_dir = rel['target']['direction']

                # Calculate connection points based on these directions
                source_point = calculate_connection_points(source_element['bounds'], source_dir)
                target_point = calculate_connection_points(target_element['bounds'], target_dir)
                ...
                # Update relationship
                relationships[rel_id]['source']['bounds'] = {
                    "x": source_point['x'],
                    "y": source_point['y'],
                    "width": 0,
                    "height": 0
                }
                relationships[rel_id]['target']['bounds'] = {
                    "x": target_point['x'],
                    "y": target_point['y'],
                    "width": 0,
                    "height": 0
                }
                relationships[rel_id]['bounds'] = rel_bounds
                relationships[rel_id]['path'] = path_points

    # Optimize paths to avoid crossing through elements
    relationships = optimize_path_routing(elements, relationships)

    # Helper function to detect hierarchy interruptions
    def check_hierarchy_interruptions(elements, relationships):
        """Check if non-inheritance elements interrupt inheritance hierarchies."""
        # Find all inheritance relationships
        inheritance_rels = {
            rel_id: rel for rel_id, rel in relationships.items()
        ...
        interruptions = []
        parent_child_map = {}
        for rel in inheritance_rels.values():
        ...
        # Check each parent-children group
        for parent_id, children_ids in parent_child_map.items():
            parent = elements[parent_id]
            parent_y = parent['bounds']['y']

            children_y_values = [elements[child_id]['bounds']['y'] for child_id in children_ids]
            min_child_y = min(children_y_values)
            max_child_y = max(children_y_values)

            # Find elements between parent and children
            for elem_id, elem in elements.items():
                if elem.get('owner') is None and elem.get('type') in ['Class', 'AbstractClass', 'Enumeration', 'ClassOCLConstraint']:
                    elem_y = elem['bounds']['y']
                    # Check if this element is between parent and children
                    if elem_id != parent_id and elem_id not in children_ids:
                        if parent_y < elem_y < min_child_y or parent_y > elem_y > max_child_y:
                            interruptions.append((parent_id, elem_id, children_ids))

        return interruptions

    # Iterative refinement: Check for collisions and hierarchy interruptions, reapply layout if needed
    # This handles cases where hierarchy interruptions or collisions cause suboptimal layouts
    max_iterations = 2
    iteration = 0

    while iteration < max_iterations:
        # Check for collisions and hierarchy interruptions
        collisions = detect_collisions(elements)
        hierarchy_interruptions = check_hierarchy_interruptions(elements, relationships)

        if not collisions and not hierarchy_interruptions:
            # No issues, layout is good
            break

        # Issues detected, apply layout again
        iteration += 1
        elements, relationships = layout_engine.calculate_layout(elements, relationships)
        relationships = optimize_path_routing(elements, relationships)
