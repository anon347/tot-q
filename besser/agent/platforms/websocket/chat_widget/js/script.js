...
function renderChatWidget(args) {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (!msg || typeof msg === 'string' &&  msg.includes("session-id:")) {
            //console.log("msg received:", msg)
            return
        }
    ...
    messageInput.addEventListener('keydown', (event) => {
      if (event.key === 'Enter') {
        /*console.log('1. send json, 2. send messge');
        // Send message to iframe
        const iframe = document.getElementById('myIframe');
        const messageToSend = 'plantuml_data_here';*/
        console.log('Adding json to message');

        const inputField = document.getElementById('message-input');
        let userMessage = inputField.value.trim();

        if (!userMessage) 
            console.warn('Empty json.');
        else
            console.log('Sending json:', userMessage);
    ...
    input.onchange = function(event) {
      ...
      if (file) {        
        ...
        reader.onload = function(e) {
          ...
          // Send file data to Streamlit (parent window)
          window.parent.postMessage({ action: 'upload_file', message }, '*');
          console.log('File send:', file.name);
...
function sendMessage() {
  /*if (!latestDiagramJson) {
    console.warn('No input json.');
  } else {
    if (latestDiagramJson) {
        let parsedJson;
        try {
        // Only parse if it's a string
        parsedJson = typeof latestDiagramJson === 'string'
            ? JSON.parse(latestDiagramJson)
            : latestDiagramJson;;

        } catch (e) {
        console.error('❌ Error parsing diagram JSON:', e, latestDiagramJson);
        }
        console.log('Sending json frontend:')//, latestDiagramJson);
        const payload = {
        action: 'user_message',
        message: parsedJson,
        };
...
function getMessageImage(message) {
    // message es un objeto { name, type, base64 }
    const base64data = typeof message === 'string' ? message : message.base64;
...
function getMessageFile(message) {
    ...
    downloadLink.classList.add('options-message');
...
function displayMessage(payload, className) {
    ...
    // hideTypingIndicator();
    if (['agent_reply_file', 'user_file'].includes(payload.message.action) && payload.message.message) {
        console.log(payload)
        messageElement = getMessageFile(payload.message.message);
    }
    else if (payload.message.action === 'agent_reply_image' && payload.message.message) {
        messageElement = getMessageImage(payload.message.message);
    }
    else if (['agent_reply_str', 'user_message'].includes(payload.action) && payload.message) {
    ...
    else if (['agent_reply_file', 'user_file'].includes(payload.message.action) && payload.message.message) {
        messageElement = getMessageFile(payload.message.message);
    }
...
function toggleChatWindow() {
  ...
  // --- REPORT HEIGHT TO PARENT ---
    //const height = expanded ? 709 : 150;
    const height = expanded
    window.parent.postMessage({ type: 'chat-height', height }, "*");
}
