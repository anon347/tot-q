...
export const useImportDiagramFromText = () => {
  const dispatch = useAppDispatch();
  const navigate = useNavigate();
  const convertBumlToDiagram = useBumlToDiagram();


  const importDiagramFromText = useCallback(async (text : any) => {
    try {
      if (Object.prototype.toString.call(text) !== '[object String]') {
        throw new Error('Input is not a valid string.');
      }

      text = await convertBumlToDiagram(text);

      const trimmedText = text.trim();
      if (!isJsonString(trimmedText)) {
        throw new Error('Provided text is not valid JSON.');
      }

      const jsonModel = JSON.parse(trimmedText);

      // Ensure the model has a type
      if (!jsonModel || !jsonModel.type) {
        throw new Error('Invalid diagram model: missing type.');
      }

      const diagram: Diagram = {
        id: uuid(),
        title: jsonModel.title || 'Imported Diagram',
        model: jsonModel,
        lastUpdate: new Date().toISOString(),
        description: 'Imported from raw JSON string',
      };
      ...
      console.error('Error importing diagram from text:', error);
      dispatch(
        displayError('Import failed', `Could not import from text: ${errorMessage}`)
      );
    }
  }, [dispatch, navigate]);

  return importDiagramFromText;
};

  /*
export const importDiagramTxt = async (text: string, editor: ApollonEditor | null) => {
    console.log('text error:', text, 'type:', typeof text);
    // Check if text is a primitive string
    if (Object.prototype.toString.call(text) !== '[object String]') {
        console.error('text is not a primitive string:', text);
        return; // Early return if text is not a primitive string
    }
    let strippedText = text.trim();
    console.log(`Text after trim: '${strippedText}'`);
        
    const isJson = isJsonString(strippedText);
    //if (!editor || strippedText === '') return;
    if (!editor || !text || !isJsonString(strippedText)) return;
    try {
        ...
        console.error('Error importing diagram:', error);
        throw new Error('Error importing diagram. Please check if the file is valid text with JSON.');
    }
};*/