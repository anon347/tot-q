...
import { useImportDiagramFromText } from '../../../services/import/useImportDiagram';
//import { useImportDiagramFromText } from '../../../services/import/useImportDiagramFromText'; 
...
/*async function convert(file) {
  try {
    ...
    console.log('BUMl file successfully converted and editor updated.');
  } catch (error) {
    console.error('Failed to convert BUMl file:', error);
  }
}
...
if (typeof window !== 'undefined') {
  window.addEventListener('message', async (event) => {  
    console.log('Message received from Streamlit in public generate:', event.data);
    importDiagramTxt(event.data, editor);
  });
}*/
...
/*
export const DiagramImportToText: React.FC = () => {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const { currentProject } = useProject();
  const handleImportDiagramToProject = useImportDiagramToProjectWorkflow();
