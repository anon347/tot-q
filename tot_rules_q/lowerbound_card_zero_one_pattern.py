...
class LowerboundCardinalityOneVsManyConfiguration(Configuration):
    ...
    def update(self, cfg, domain_model, check_option):
        ...
        if (check_option == "Option 1" and cfg.option_1_dm) or \
            ...
            if check_option == "Option 1":
                ...
                if cfg.direction == 'source':
                    for rel in rel_to_update:
                        if rel:
                            rel.sourceCardinality.set_min_score(high_confidence)
                elif cfg.direction == 'target':
                    for rel in rel_to_update:
                        if rel:
                            rel.targetCardinality.set_min_score(high_confidence)
                cfg.resulting_element = ('lowerbound_cardinality', alt1)
            else:
                ...
                if cfg.direction == 'source':
                    ...
                    for rel in rel_to_update:
                        if rel:
                            rel.targetCardinality.set_min_score(high_confidence)
                cfg.resulting_element = ('lowerbound_cardinality', alt2)
            return None
            ...
            cfg.resulting_element = ('lowerbound_cardinality', alt1_dm.relationships[0])
            return domain_model
            ...
            cfg.resulting_element = ('lowerbound_cardinality', alt2_dm.relationships[0])
            return domain_model
...
def setup_lowerbound_cardinality_zero_vs_one_patterns(domain_model, domain_model_alternatives, template_module=None):
    configurations = []
    ...
    if template_module:
        generate_target_lowerbound_cardinality_zero_question = template_module.generate_target_lowerbound_cardinality_zero_question
    else:
        generate_target_lowerbound_cardinality_zero_question = get_question_function('generate_target_lowerbound_cardinality_zero_question')

    target_zero_conf_alt, target_zero_conf_no_alt = find_lowerbound_zero_alternatives(
        target_lowerbound_cardinality_zero_alternatives, target_lowerbound_cardinality_one_dm, domain_model,
        question_generator_fn=generate_target_lowerbound_cardinality_zero_question
    )

    if template_module:
        generate_target_lowerbound_cardinality_one_question = template_module.generate_target_lowerbound_cardinality_one_question
    else:
        generate_target_lowerbound_cardinality_one_question = get_question_function('generate_target_lowerbound_cardinality_one_question')

    target_one_conf_alt, target_one_conf_no_alt = find_lowerbound_one_alternatives(
        target_lowerbound_cardinality_zero_dm, target_lowerbound_cardinality_one_alternatives, domain_model,
        question_generator_fn=generate_target_lowerbound_cardinality_one_question
    )
    ...
    if template_module:
        generate_source_lowerbound_cardinality_zero_question = template_module.generate_source_lowerbound_cardinality_zero_question
    else:
        generate_source_lowerbound_cardinality_zero_question = get_question_function('generate_source_lowerbound_cardinality_zero_question')

    source_zero_conf_alt, source_zero_conf_no_alt = find_lowerbound_zero_alternatives(
        source_lowerbound_cardinality_zero_alternatives, source_lowerbound_cardinality_one_dm, domain_model,
        question_generator_fn=generate_source_lowerbound_cardinality_zero_question
    )

    if template_module:
        generate_source_lowerbound_cardinality_one_question = template_module.generate_source_lowerbound_cardinality_one_question
    else:
        generate_source_lowerbound_cardinality_one_question = get_question_function('generate_source_lowerbound_cardinality_one_question')

    source_one_conf_alt, source_one_conf_no_alt = find_lowerbound_one_alternatives(
        source_lowerbound_cardinality_zero_dm, source_lowerbound_cardinality_one_alternatives, domain_model,
        question_generator_fn=generate_source_lowerbound_cardinality_one_question
    )
